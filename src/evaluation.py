...
def precision_at_k(relevant_docs: Sequence[DocID], retrieved_docs: Sequence[DocID], k: int) -> float:
    retrieved_k = retrieved_docs[:k]
    relevant_retrieved = len(set(relevant_docs).intersection(set(retrieved_k)))
    return relevant_retrieved / k

def recall_at_k(relevant_docs: Sequence[DocID], retrieved_docs: Sequence[DocID], k: int) -> float:
    retrieved_k = retrieved_docs[:k]
    relevant_retrieved = len(set(relevant_docs).intersection(set(retrieved_k)))
...
def map_at_k(queries: Sequence[str], query_results: Dict[QueryID, DocID], index_directory: str, k: int, query_function: Callable=query_database) -> float:
    ...
    for q_id, q in tqdm(queries.items(), desc=f"Processing queries for MAP@K={k}"):
        ...
        retrieved_docs = [doc_id for doc_id, _ in query_function(index_directory, q, top_k=k)]
...
def mar_at_k(queries: Sequence[str], query_results: Dict[QueryID, DocID], index_directory: str, k: int, query_function: Callable=query_database) -> float:
...
def retrieve_top_k_docs(
    ...
    """
    top_k_docs = {}
    for query_id, query_str in tqdm(queries.items(), desc=f"Retrieving top-{k} documents"):
        # Retrieve the top k documents for the query
        retrieved_docs = [doc_id for doc_id, _ in query_function(index_directory, query_str, k)]
